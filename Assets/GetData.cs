...
public class GetData : MonoBehaviour
{
    ...
    public BinDataSave BinDataSave;
    ...
    void Read_Data(byte input)
    {
        ...
        else if (decode_state == 36)
        {
            ...
            if (signal_count < 101)
            {
                ...
                for (int k = 0; k < 34; k++)
                {
                    BinDataSave.SaveList.Add(DATA[k]);
                }
                for (int k = 0; k < 17; k++)
                {
                    BinDataSave.SaveSignal.Add(signal[k]);
                }
            }
            else
            {
                BinDataSave.BinSave(BinDataSave.SaveList, "1_1_L");
                BinDataSave.BinSaveSignal(BinDataSave.SaveSignal, "1_2_L");
                signal_count = 1;
                //print("OK");
            }
